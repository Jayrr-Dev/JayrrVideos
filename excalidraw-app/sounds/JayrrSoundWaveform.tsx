...
const readTone = (node: HTMLElement, playing: boolean) => {
  ...
  const rest =
    styles?.getPropertyValue("--color-gray-40").trim() || "#9ca3af";
  const played =
    styles?.getPropertyValue("--color-primary").trim() || "#6965db";
  return playing ? played : rest;
};

export const JayrrSoundWaveform = memo(function JayrrSoundWaveform({
  sources,
  tone = "idle",
  progress = 0,
  start = 0,
  ...
  useLayoutEffect(() => {
    ...
    const draw = () => {
      ...
      for (let i = 0; i < barCount; i += 1) {
        const amp = Math.max(1, (bars[i] ?? 0) * (thick * 0.48));
        const played = i / barCount < progress;
        const fill = played ? color : withAlpha(color, tone === "playing" ? 1 : 0.85);
        if (axis === "x") {
          const top = mid - amp;
  ...
  }, [axis, end, progress, start, tone, version]);
