...
export type BackgroundMode = 'blur' | 'image' | 'color' | 'transparent' | 'none';
...
export class SegmentationProcessor {
  ...
  async init(width: number, height: number): Promise<void> {
    ...
    this.pipeline = new PostProcessingPipeline({
      ...
      lightWrap: this.opts.backgroundMode === 'transparent' ? false : preset.lightWrap,
  ...
  processFrame(frame: TexImageSource, timestamp: number): OffscreenCanvas | null {
    ...
    // Auto-frame is off for cutouts. Skip the crop lookup on that path.
    if (this.opts.autoFrame?.enabled) {
