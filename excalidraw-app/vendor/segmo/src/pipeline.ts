...
export interface PipelineOptions {
  ...
  maskHeight: number;
  /** Background mode: 'blur' | 'image' | 'color' | 'transparent' */
  backgroundMode: "blur" | "image" | "color" | "transparent";
...
export class PostProcessingPipeline {
  ...
  private cropRect: { x: number; y: number; w: number; h: number } | null =
    null;
  ...
  private init(): void {
    ...
    this.temporalProg = this.createProgram(
      VERTEX_SHADER,
      TEMPORAL_SMOOTH_SHADER,
      [
        "u_currentMask",
    ...
    this.bilateralProg = this.createProgram(
      VERTEX_SHADER,
      BILATERAL_UPSAMPLE_SHADER,
      [
        "u_mask",
    ...
    this.colorMatchProg = this.createProgram(
      VERTEX_SHADER,
      COLOR_MATCH_SHADER,
      [
        "u_background",
    ...
    this.compositeFBO = this.opts.lightWrap
      ? this.createFramebuffer(width, height)
      : this.createFramebuffer(1, 1);
    this.preCropFBO = this.createFramebuffer(1, 1);

    // Blur at half resolution for performance. Color/transparent skip the
    // 6-pass Gaussian, so keep these FBOs tiny until blur mode is used.
    if (this.opts.backgroundMode === "blur") {
      ...
      this.blurFBO1 = this.createFramebuffer(1, 1);
      this.blurFBO2 = this.createFramebuffer(1, 1);
    }
  ...
  process(
    ...
    this.renderToFBO(this.temporalFBO, this.temporalProg, () => {
      ...
      this.bindTexture(
        2,
        hasMotionMap ? this.motionTexture : this.maskTexture,
        "u_motionMap",
      );
      gl.uniform1f(
        this.temporalProg.uniforms["u_appearRate"],
        this.isFirstFrame ? 1.0 : this.opts.appearRate,
      );
      gl.uniform1f(
        this.temporalProg.uniforms["u_disappearRate"],
    ...
    if (this.opts.morphology) {
      ...
      this.renderToFBO(this.morphologyFBO1, this.morphologyProg, () => {
        ...
        gl.uniform1f(
          this.morphologyProg.uniforms["u_radius"],
          this.opts.morphologyRadius,
        );
      });
    ...
    this.renderToFBO(this.bilateralFBO, this.bilateralProg, () => {
      ...
      gl.uniform2f(
        this.bilateralProg.uniforms["u_maskSize"],
        maskWidth,
        maskHeight,
      );
    ...
    this.renderToFBO(this.featherFBO, this.featherProg, () => {
      ...
      gl.uniform2f(
        this.featherProg.uniforms["u_texelSize"],
        1.0 / width,
        1.0 / height,
      );
      gl.uniform1f(
        this.featherProg.uniforms["u_featherRadius"],
        this.opts.featherRadius,
      );
    ...
    if (this.opts.backgroundMode === "blur") {
      ...
      this.opts.backgroundMode === "image" &&
      this.opts.backgroundImage
    ) {
    ...
    const renderComposite = compositeTarget
      ? (setup: () => void) =>
          this.renderToFBO(compositeTarget, this.compositeProg, setup)
      : (setup: () => void) => this.renderToScreen(this.compositeProg, setup);

    const crop =
      this.opts.backgroundFixed && this.cropRect
        ? this.cropRect
        : { x: 0, y: 0, w: 1, h: 1 };
  ...
  processInterpolated(
    ...
    const hasShift =
      maskShift &&
      (Math.abs(maskShift.dx) > 0.0001 || Math.abs(maskShift.dy) > 0.0001);
    ...
    if (hasShift) {
      this.renderToFBO(this.shiftFBO, this.shiftProg, () => {
        ...
        gl.uniform2f(
          this.shiftProg.uniforms["u_shift"],
          maskShift!.dx,
          maskShift!.dy,
        );
      });
  ...
  setCropRect(
    ...
    this.cropRect = rect;
    if (rect && this.preCropFBO.width !== this.opts.width) {
      const gl = this.gl;
      gl.deleteFramebuffer(this.preCropFBO.fbo);
      gl.deleteTexture(this.preCropFBO.texture);
      this.preCropFBO = this.createFramebuffer(
        this.opts.width,
        this.opts.height,
      );
    }
  ...
  private backgroundModeIndex(): number {
    if (this.opts.backgroundMode === "blur") {
      return 0;
    }
    if (this.opts.backgroundMode === "image") {
      return 1;
    }
    if (this.opts.backgroundMode === "transparent") {
      return 3;
    }
    return 2;
  }
  ...
  private generateBlurredBackground(cameraFrame: TexImageSource): void {
    const blurW = Math.floor(this.opts.width / 2);
    const blurH = Math.floor(this.opts.height / 2);
    if (this.blurFBO1.width !== blurW || this.blurFBO2.height !== blurH) {
      const gl = this.gl;
    ...
    for (let i = 0; i < 2; i++) {
      this.renderToFBO(this.blurFBO1, this.blurProg, () => {
        ...
        gl.uniform2f(this.blurProg.uniforms["u_direction"], 1.0 / blurW, 0.0);
        gl.uniform1f(
          this.blurProg.uniforms["u_radius"],
          this.opts.blurRadius * 0.7,
        );
      });
  ...
  private createProgram(
    ...
    if (!gl.getShaderParameter(vertShader, gl.COMPILE_STATUS)) {
      throw new Error(
        `Vertex shader error: ${gl.getShaderInfoLog(vertShader)}`,
      );
    }
    ...
    if (!gl.getShaderParameter(fragShader, gl.COMPILE_STATUS)) {
      throw new Error(
        `Fragment shader error: ${gl.getShaderInfoLog(fragShader)}`,
      );
    }
