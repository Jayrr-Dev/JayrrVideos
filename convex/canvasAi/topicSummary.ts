...
export function validateTopicSummary(
  value: unknown,
  ...
  for (const update of output.topics) {
    ...
    if (
      !original ||
      seen.has(update.id) ||
      update.turnIds.some((id) => !original.turnIds.includes(id))
    ) {
      throw new Error("Summary contains unsupported evidence references.");
    }
    seen.add(update.id);
  }
  if (seen.size !== input.topics.length) {
    throw new Error("Summary omitted a topic.");
  }
...
export async function summarizeTopics(raw: string) {
  if (raw.length > 60000) {
    throw new Error("Summary input is too large.");
  }
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(`Topic summary unavailable (${response.status}).`);
    }
    const data = await response.json();
    const content = data.choices?.[0]?.message?.content;
    if (typeof content !== "string") {
      throw new Error("Summary returned no content.");
    }
